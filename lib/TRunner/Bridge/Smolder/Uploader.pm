...
# ABSTRACT: wtf.. idon't have any idea
use utf8;
use Any::Moose;
...
has [ 'server', 'project_id', 'username', 'password', 'file' ] =>
  ( is => 'ro', required => 1 );
has [ 'comments', 'architecture', 'platform', 'tags', 'revision' ] =>
  ( is => 'ro' );
...
sub upload {
    ...
    try {
        $self->_mech->get(
            $self->base_url . '/projects/add_report/' . $self->project_id );
        $self->_mech->form_name('add_report');
        my %fields = ( report_file => $self->file );
        map { $fields{$_} = $self->$_ }
          grep { $self->$_ } qw/platform architecture tags comments revision/;
